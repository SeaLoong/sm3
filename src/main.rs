...
// 文件中对算法的描述是按bit来操作，实现上这里不接受bit操作，最小操作单位为u8
...
fn SM3(mut m: Vec<u8>) -> String {
    m = padding(m);
    let v = iterate(m);
    v.into_iter().map(|x| format!("{:08x}", x)).collect::<Vec<String>>().join(" ")
}
...
\t\t path: Input file.\n\
\t-h, --help: Show help.\n\
\t-s, --str <string>: Input a string instead of file.\n\
";
...
fn read_file(path: String) -> Vec<u8> {
    std::fs::read(&path).expect(&format!("Can't read file '{}'", path))
}
...
fn test() {
    ...
    let result = SM3(m);
    println!("{}", result);
    assert_eq!("66c7f0f4 62eeedd9 d1f2d46b dc10e4e2 4167c487 5cf2f7a2 297da02b 8f4ba8e0", result);
    let m = "abcd".repeat(16).as_bytes().to_vec();
    let result = SM3(m);
    println!("{}", result);
    assert_eq!("debe9ff9 2275b8a1 38604889 c18e5a4d 6fdb70e5 387e5765 293dcba3 9c0c5732", result);
}
